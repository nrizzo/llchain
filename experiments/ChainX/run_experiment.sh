...
check=$(diff \
	<(grep "distance =" time_global_chainx     | cut -d'=' -f2 | cut -d')' -f2 | tr -d " ") \
	<(grep -v "^\[" time_global_llchain | grep -v "^#" | grep -v "maxresident" | cut -f3))
if [ "$check" != "" ] ; then echo " it differs!" ; exit 1 ; fi
...
grep -v "^\[" time_global_llchain | grep -v "^#" | grep -v "maxresident" | cut -f10 | tr -d "s" | tac >> stats_time_time_global_llchain
...
check=$(diff \
	<(grep "distance =" time_semiglobal_chainx     | cut -d'=' -f2 | cut -d')' -f2 | tr -d " ") \
	<(grep -v "^\[" time_semiglobal_llchain | grep -v "^#" | grep -v "maxresident" | cut -f3))
if [ "$check" != "" ] ; then echo " it differs!" ; exit 1 ; fi
echo " done (no difference)."
...
grep -v "^\[" time_semiglobal_llchain | grep -v "^#" | grep -v "maxresident" | cut -f10 | tr -d "s" | tac >> stats_time_time_semiglobal_llchain
...
do
	...
	check=$(diff \
		<(grep "distance =" correlation_global_chainx_$m     | cut -d'=' -f2 | cut -d')' -f2 | tr -d " ") \
		<(grep -v "^\[" correlation_global_llchain_$m | grep -v "^#" | grep -v "maxresident" | grep -v "elapsed" | cut -f3))
	if [ "$check" != "" ] ; then echo " it differs!" ; exit 1 ; fi
done
echo " done (no difference)."
...
do
	...
	check=$(diff \
		<(grep "distance =" correlation_semiglobal_chainx_$m     | cut -d'=' -f2 | cut -d')' -f2 | tr -d " ") \
		<(grep -v "^\[" correlation_semiglobal_llchain_$m  | grep -v "^#" | grep -v "maxresident" | grep -v "elapsed" | cut -f3))
	if [ "$check" != "" ] ; then echo " it differs!" ; exit 1 ; fi
done
echo " done (no difference)."
