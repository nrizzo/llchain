...
for m in 70 80 90 94 97 99
do
	$usrbintime "$usrbintimeoptions" $clcviz $clcvizmode -t $ref -q "$thisfolder/../../ext/ChainX/data/time_global/mutated_${m}_perc.fasta" \
		>> time_global_clc-viz 2>> time_global_clc-viz
done
...
for m in 70 80 90 94 97
do
	$usrbintime "$usrbintimeoptions" $clcviz $clcvizmode -t $ref -q "$thisfolder/../../ext/ChainX/data/time_semiglobal/mutated_${m}_perc.fasta" \
		>> time_semiglobal_clc-viz 2>> time_semiglobal_clc-viz
done
...
for m in "75_80" "80_90" "90_100"
do
	$usrbintime "$usrbintimeoptions" $clcviz $clcvizmode -t $ref -q "$thisfolder/../../ext/ChainX/data/correlation_global/count100_mutated_$m.fa.gz" \
		>> correlation_global_clc-viz_$m 2>> correlation_global_clc-viz_$m
done
...
for t in correlation_global_chainx correlation_global_chainx_opt correlation_global_clc-viz
do
...
for m in "75_80" "80_90" "90_100"
do
	$usrbintime "$usrbintimeoptions" $clcviz $clcvizmode -t $ref -q "$thisfolder/../../ext/ChainX/data/correlation_semiglobal/count100_mutated_$m.fa" \
		>> correlation_semiglobal_clc-viz_$m 2>> correlation_semiglobal_clc-viz_$m
done
...
for t in correlation_semiglobal_chainx correlation_semiglobal_chainx_opt correlation_semiglobal_clc-viz
do
	echo "time (s)" > stats_correlation_time_$t
	echo "space (kb)" > stats_correlation_space_$t
done
